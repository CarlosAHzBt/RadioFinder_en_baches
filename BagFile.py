...
class BagFile:
    ...
    def process_bag_file(self):
        """
        Procesa el archivo .bag extrayendo y guardando los frames y las nubes de puntos alineadas.
        """
        pipeline = rs.pipeline()
        config, align = self.configure_pipeline(pipeline)  # Guardamos también el objeto align
        self.extract_and_save_frames(pipeline, align)

    def configure_pipeline(self, pipeline):
        """
        Configura la pipeline de RealSense para leer desde el archivo .bag y crea un objeto de alineación.
        """
        config = rs.config()
        config.enable_device_from_file(self.bag_file_path, repeat_playback=False)
        profile = pipeline.start(config)
        align = rs.align(rs.stream.color)  # Crear objeto de alineación para alinear a la cámara de color
        return config, align

    def extract_and_save_frames(self, pipeline, align):
        """
        Extrae, procesa, alinea y guarda los frames y las nubes de puntos del archivo .bag.
        """
        align = rs.align(rs.stream.color)  # Crear objeto de alineación
        pc = rs.pointcloud()  # Crear objeto para procesamiento de nube de puntos
        frame_number = 0
        try:
            while True:
                ...
                self.save_depth_frame_as_ply(color_frame, depth_frame, frame_number,aligned_frames)
                frame_number += 1
    ...
    def save_depth_frame_as_ply(self, color_frame, depth_frame, frame_number,aligned_frames):
        """
        ...
        aligned_points = pc.calculate(depth_frame)
